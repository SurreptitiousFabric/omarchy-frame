...
- Code and packaging candidate: public commit `9a6d561`
- Installed public checkout: exact candidate `9a6d561` from public GitHub
...
| Backend statement coverage at least 75% | PASS | 76.3% backend / 76.4% repository total on `9a6d561` |
| Explicit remote click/hold allowlists and QML parity | PASS | Automated contract tests |
| QML parses, composes, and renders in Omarchy shell | PASS | `9a6d561` passed native type contracts, fail-closed mutations, explicit visible icon/click-target composition, 24/32-pixel image assertions, headless-Sway layer-shell loading, two-monitor installed geometry, panel summon/hide, fresh journal inspection, clean-clone validation, and the required Arch CI job |
...
| Deterministic portable release archive | PASS | Exact `9a6d561` tests prove identical repeated archives, normalized metadata, an exact launcher/binaries/checksum allowlist, and byte stability across evidence-only commits; CI packaged and uploaded the archive |
...
| Official Go vulnerability analysis | PASS | `govulncheck` v1.7.0 reported no vulnerabilities for `9a6d561` locally, in a clean clone, and in CI |
...
| Clean Git checkout validates and runs packaged backend | PASS | Separate untrusted `--no-local` clone of `9a6d561`; race, vet, vulnerability, ShellCheck, workflow, release-policy, archive, type, QML policy, widget composition, icon pixels, manifest, repository policy, checksum, and no-Go gates passed and left it clean |
...
| Marketplace static security baseline | PASS | Marker protocol v4 / policy 3 on public `9a6d561`: one valid root plugin, zero findings, non-blocking `review-required`, and expected `bundled-executable-binary` capability only; an initial anonymous GitHub 504 failed closed before an authenticated retry completed on the same metadata and SHA |
| CI workflow executes remotely | PASS | GitHub Actions run `33053124957` passed required `qml-contract` and `verify` jobs on exact `9a6d561`, including headless layer-shell loading, rendered pixels, deterministic package upload, and no invalid Qt backend warning |
...
| Release guard rejects premature or incoherent tags | PASS | Exact `9a6d561` mutation tests require stable v1+ tag/manifest/acceptance/changelog agreement, a clean tree, candidate ancestry, evidence-only post-candidate changes, and no applicable pending row except the workflow being exercised; a restricted-PATH case proves runner portability |
...
remote run. Bar-coverage candidate `8fdef22` failed run `33052231494` because
the new runtime test had no headless display. Candidate `dc11cb1` added
headless Sway but failed run `33052737390` because Arch's optional
`cap_sys_nice` file capability cannot execute in the restricted container.
Candidate `6e8dad0` passed run `33052944241` after removing that capability in
the ephemeral job, but log review found an invalid Qt software-backend key.
Final candidate `9a6d561` uses Qt's supported software adaptation and passed the
clean run cited above. No failed or superseded candidate is release evidence.

## Install and shell integration

| Check | Status | Evidence |
|---|---|---|
| Public GitHub install through `omarchy plugin add` | PASS | Removed the local-source clone, installed public commit `1d5d949` from the documented GitHub URL, then updated through the same public origin to exact candidate `9a6d561`; a transient initial Omarchy discovery race required one rescan/retry |
...
| Panel opens without Frame-specific QML/runtime error | PASS | Installed exact candidate passed visible icon/click-target composition and pixel rendering; supported shell IPC reported two visible 30×30 instances, summon/hide returned successfully, and the resulting journal window contained zero Frame errors |
