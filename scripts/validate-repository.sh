...
while IFS= read -r executable; do
  case $executable in
    ./bin/frame-controller|./bin/frame-controller-linux-amd64|./bin/frame-controller-linux-arm64|./scripts/build-release.sh|./scripts/test-packaged-runtime.sh|./scripts/test-repository-policy.sh|./scripts/validate-repository.sh) ;;
    *)
...
bash -n scripts/build-release.sh scripts/test-packaged-runtime.sh scripts/test-repository-policy.sh scripts/validate-repository.sh
