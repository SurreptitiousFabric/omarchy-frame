...
while IFS= read -r executable; do
  case $executable in
    ./bin/frame-controller|./bin/frame-controller-linux-amd64|./bin/frame-controller-linux-arm64|./scripts/*.sh) ;;
    *)
...
mapfile -t repository_scripts < <(find scripts -maxdepth 1 -type f -name '*.sh' -print | sort)
